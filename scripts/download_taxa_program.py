...
class download_w_taxa:
    ...
    def __init__(self, search_statement, genus):
        ...
        self.genus = genus
        self.path = f"/home/biokate/thesis_unix/test_data/scripts{self.genus}/{self.genus}.fa"
        self.path2 = f"/home/biokate/thesis_unix/test_data/scripts{self.genus}/{self.genus}_reps.fa"
    ...
    def create_fasta(self):
        ...
        if check != 0:
            ...
            os.system(f"mkdir /home/biokate/thesis_unix/test_data/scripts{self.genus}")
    ...
    def make_lineage_file(self):
        for entry in self.faster_reader2():
            os.system(f"""esearch -db nucleotide -query "{entry.id}" | esummary | xtract -pattern TaxId -element TaxId >> /home/biokate/thesis_unix/test_data/scripts{self.genus}/tax_id.txt""")


        with open(f'/home/biokate/thesis_unix/test_data/scripts{self.genus}/tax_id.txt') as index:
            test = index.read()
        ...
        output = open(f"/home/biokate/thesis_unix/test_data/scripts{self.genus}/{self.genus}_lin.txt", "w")
        for key, value in lineage.items():
            output.write('%s:%s\n' % (key, value))

        output.close()
        with open(f"/home/biokate/thesis_unix/test_data/scripts{self.genus}/{self.genus}_lin.txt", "r") as file:
            contents = file.read()
            n = 0
            if "Chlorophyta" in contents:  # Check for green algae taxonomy
                n = 1  # Indicates that the correct taxonomy is present
            else:
                n = 0  # Indicates that the desired taxonomy was not found
            return n
